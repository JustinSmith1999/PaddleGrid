                  ...
                  <React.Fragment key={index}>
                    {parts.map((part, partIndex) => {
                      if (part.match(urlRegex)) {
                        // Render URLs as inline text links — explicit action
                        // chips appear once in the quick-action row below.
                        return (
                          <a
                            key={partIndex}
                            href={part}
                            ...
                            className="text-emerald-700 hover:text-emerald-900 underline underline-offset-2 break-all"
                          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-24">
          <div className="lg:col-span-2 space-y-6">
            <AnimatePresence mode="wait">
              {/* Courts tab — PodPlay-style time slot board */}
              {currentTab === 'courts' && (
                <motion.div
                  key="courts"
                  initial="hidden"
                ...
                >
                  <SlotBoard
                    ...
                    onBookSlot={(date, startTime, ids) => {
                      if (!user) { alert('Please sign in to book a court'); return; }
                      if (ids.length === 0) return;
                      setSelectedCourtId(ids[0]);
